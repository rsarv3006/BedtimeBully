...
///         }
///     }
public struct BedtimeHistoryRequest: Queryable {
    // MARK: - Queryable Implementation
    public init () {}
    
    public static var defaultValue: [GRDBBedtimeHistory] { [] }
    
    public func publisher(in appDatabase: AppDatabase) -> AnyPublisher<[GRDBBedtimeHistory], Error> {
    ...
    func fetchValue(_ db: Database) throws -> [GRDBBedtimeHistory] {
            return try GRDBBedtimeHistory.all().fetchAll(db)
    }
}
